Ext.ns("Ext.HY.BLCheckView");
var blchkView = Ext.HY.BLCheckView;

Ext.apply(blchkView, {
	ctx: ctx + '/jobshp/blchk', pkName: 'shpblid', codeName: 'cnorname', title: '提单补料'
});
...
Ext.onReady(function() {
	...
	var viewport = new Ext.Viewport({
		layout: 'border',
		items: [
			blchkView.queryPanel,
			blchkView.mainPanel
		]
	});
	viewport.doLayout();
	
	if (HY.isNumber(to_pkval)) {
		blchkView.doModify(to_pkval);
//		blchkView.doNewDb();
	}
	
	blchkView.cnttypeStore.load();
	blchkView.packingStore.load();
});

// ------------------ 零部件
blchkView.mainStore = HY.Cmp.createJsonStore({
	fields: ['shpblid','jobno','blnoh','blnom','clientid','vessel','voyage','etd','eta','polid','polcode','pol','podid','podcode','pod','pldid','pldcode','placedel','potid','potcode','pot','gdshscode'],
	url: blchkView.ctx + '/datlst.do',
	sortInfo: {field: 'blnoh', direction: 'ASC'}
});
blchkView.mainCM = new Ext.grid.ColumnModel({
	columns:[
		...
		{header: '航次', dataIndex: 'voyage', width: 100, sortable: true},
...
blchkView.mainFields = [
	...
	{layout: 'absolute', height: 680, border: true, items:[
		...
		{xtype: 'label', html: '承运船东/Carrier', width: 200, x: 335, y: 7, cls: 'x-required'},{xtype: 'hidden', name: 'shpnid'},{xtype: 'hidden', name: 'shpntitle'},
		{xtype: 'textfield', name: 'shpnabbr', width: 200, x: 335, y: 25, tabIndex: 0, allowBlank: false, readOnly: true},
		HY.Cmp.createQryClrButton({btn1:{x: 538, y: 25, fn: function(){ HY.Const.popClient({id: 'shipper', comboid: 'shipper', callback: blchkView.setShipper}); }}, btn2:{x: 563, y: 25, fn: function(){ blchkView.clearPop('shpn'); }}}),
//		{xtype: 'textfield', name: 'shpntitle', width: 200, x: 335, y: 25, tabIndex: 205, allowBlank: false},
		...
		{xtype: 'label', html: '航次/Voyage', width: 200, x: 595, y: 93, cls: 'x-required'},
...
//		{xtype: 'textfield', name: 'amssvctype', width: 200, x: 335, y: 154, tabIndex: 201, allowBlank: true},
...
//		{xtype: 'textfield', name: 'snppcc', width: 200, x: 335, y: 197, tabIndex: 201, allowBlank: false},
...
//		{xtype: 'textfield', name: 'frtterm', width: 200, x: 335, y: 240, tabIndex: 201, allowBlank: true},
		...
		HY.Cmp.createQryClrButton({btn1:{x: 538, y: 326, fn: function(){ HY.Const.popPort({callback: blchkView.setPortRec}); }}, btn2:{x: 563, y: 326, fn: function(){ blchkView.clearPop('portrec'); }}}),
		...
		HY.Cmp.createQryClrButton({btn1:{x: 798, y: 326, fn: function(){ HY.Const.popPort({callback: blchkView.setPortPol}); }}, btn2:{x: 823, y: 326, fn: function(){ blchkView.clearPop('portpol'); }}}),
		...
		HY.Cmp.createQryClrButton({btn1:{x: 538, y: 369, fn: function(){ HY.Const.popPort({callback: blchkView.setPortPot}); }}, btn2:{x: 563, y: 369, fn: function(){ blchkView.clearPop('portpot'); }}}),
		...
		HY.Cmp.createQryClrButton({btn1:{x: 798, y: 369, fn: function(){ HY.Const.popPort({callback: blchkView.setPortPod}); }}, btn2:{x: 823, y: 369, fn: function(){ blchkView.clearPop('portpod'); }}}),
		...
		HY.Cmp.createButton({x: 351, y: 438, cls: 'tbar-equal', fn: function(){blchkView.doSumMarks();}}),
		...
		HY.Cmp.createButton({x: 940, y: 5, cls: 'tbar-accept', text: '确认补料', fn: function(){blchkView.doConfirm();}}),
		...
		HY.Cmp.createQryClrButton({btn1:{x: 938, y: 283, fn: function(){ HY.Const.popPacking({id: 'm-packing', callback: blchkView.setPacking}); }}, btn2:{x: 963, y: 283, fn: function(){ blchkView.clearPop('pack'); }}}),
...
blchkView.dbCM = new Ext.grid.ColumnModel({
	columns:[
		new Ext.grid.RowNumberer({width: 36}),
		...
		{header: '状态', dataIndex: 'rowdatype', width: 60, sortable: true, renderer: blchkView.renderDtlRowdatype}
	],
	defaults: HY.Cmp.cmDefaults
});
blchkView.cnttypeStore = HY.Cmp.createJsonStore({
...
blchkView.packingStore = HY.Cmp.createJsonStore({
 ...
 * 查询面板
 */
blchkView.queryPanel = new HY.Cmp.QueryPanel({
	tarObj: blchkView,
 ...
 * 主列表面板
 */
blchkView.mainPanel = new Ext.grid.GridPanel({
	region: 'center',
	border: true,
	margins: '0 0',
	store: blchkView.mainStore,
	cm: blchkView.mainCM,
	bbar: new HY.Cmp.PagingToolbar({tarObj: blchkView, store: blchkView.mainStore}),
	listeners: {
 ...
 * 装箱资料编辑窗口
 */
blchkView.dbEditWin = new HY.Cmp.WindowEditMst ({
	tbar: new HY.Cmp.Toolbar({
		...
		setAction: function(actid) {
			if (actid == HY.Cmp.actions.New['code']) {
				...
				blchkView.doRefreshDb();
			}
		}
	}),
	title: '装箱资料编辑',
	formFields: blchkView.dbFields,
	height: 320,
	width: 800
});
/**
 * 主编辑窗口
 */
blchkView.mainWindow = new HY.Cmp.WindowEditMstDtl({
	tbar: new HY.Cmp.Toolbar({
		tarObj: blchkView,
		editMode: 1,
		enableNav: true,
		...
		actionCustom2: new Ext.Action({text: '取消确认', iconCls: 'tbar-cancel', handler: function(){
			blchkView.doUnconfirm();
		}}),
		actionCustom3: new Ext.Action({text: '另存为模板', iconCls: 'tbar-export', handler: function(){
			blchkView.doSaveAsTmpl();
		}}),
		actionCustom4: new Ext.Action({text: '从模板载入', iconCls: 'tbar-query', handler: function(){
			blchkView.doShowTmplList();
		}})
	}),
	title: blchkView.title,
	maximizable: false,
	maximized: true,
	mstRegion: 'center',
	dtlRegion: 'south',
	formFields: blchkView.mainFields,
	formHeight: 220,
	gridHeight: 200,
	gridStore: blchkView.dbStore,
	gridColModel: blchkView.dbCM,
	...
	gridEditWindow: blchkView.dbEditWin
});
/**
 * 模板编辑窗口

blchkView.tmplEditWin = new HY.Cmp.WindowEditMst ({
	tbar: new HY.Cmp.Toolbar({
		editMode: 1,
		enableNav: true,
		setAction: function(actid) {
			if (actid == HY.Cmp.actions.New['code']) {
				...
				blchkView.doModifyTmpl();
			} else if (actid == HY.Cmp.actions.Save['code']) {
				blchkView.doSaveTmpl();
			} else if (actid == HY.Cmp.actions.Delete['code']) {
				blchkView.doDeleteTmpl();
			} else if (actid == HY.Cmp.actions.Close['code']) {
				blchkView.doCloseTmpl();
			} else if (actid == HY.Cmp.actions.Prevrow['code']) {
				//
			} else if (actid == HY.Cmp.actions.Nextrow['code']) {
				//
			} else if (actid == HY.Cmp.actions.Refresh['code']) {
				blchkView.doRefreshTmpl();
			}
		}
	}),
	title: '另存为模板',
...
blchkView.doRefresh = function(){
	...
	if (me.mainWindow.isVisible()) {
		...
		if (HY.isNumber(pkval) && pkval > 0) {
			blchkView.doLoadDetail(pkval, frm);
			blchkView.doRefreshDb(pkval);
		} else {
			return;
		}
	} else {
...
blchkView.doNew = function(){
	var frm = blchkView.mainWindow.formPanel.getForm();
	HY.Cmp.clearFormData(frm);
	blchkView.mainWindow.show();
	
	HY.fv(frm, 'inused', 'Y');
	blchkView.dbStore.removeAll();
	
	frm.clearInvalid();
	frm.findField(blchkView.codeName).focus(true,500);
};
blchkView.doSave = function(){
	var me = this;
	var frm = me.mainWindow.formPanel.getForm();
	if (!frm.isValid()) {
		return;
...
blchkView.doModify = function(pktmp){
	var me = this;
	var frm = me.mainWindow.formPanel.getForm();
...
blchkView.doDelete = function(){
	var rec = this.mainPanel.getSelectionModel().getSelected();
	if( rec ) {
		var pkval = rec.get(blchkView.pkName);
		if (!HY.isNumber(pkval)) return;
		HY.confirm ({
			showWat: true,
			callback: function() {
				var url = blchkView.ctx + '/del/' + pkval + '.do';
				HY.AjaxRequest({url: url, showMsg: true, callback: function(datobj){
					blchkView.mainStore.remove(rec);
				}});
			}
		});
	}
};
blchkView.doLoadDetail = function(pkval, frm) {
	if (!HY.isNumber(pkval)) {
		HY.alert("参数有误，请重试。");
		return;
	}
	var url = blchkView.ctx + '/get/' + pkval + '.do';
...
blchkView.doClose = function() {
	this.mainWindow.hide();
};
// ------------------------------------- DB Info
blchkView.doRefreshDb = function(srvid) {
	var me = this;
	if (me.dbEditWin.isVisible()) {
		var frm = me.dbEditWin.formPanel.getForm();
		var pkval = HY.fv(frm, 'cntblid');
		var lnkid = HY.fv(frm, 'cntsrcid');
...
blchkView.doNewDb = function(){
	...
	HY.fv(frm, 'shpblid', mpkval);
	
	frm.clearInvalid();
	frm.findField('cntno').focus(true,500);
};
blchkView.doSaveDb = function(){
	var me = this;
	var frm = me.dbEditWin.formPanel.getForm();
	if (!frm.isValid()) {
		return;
	}
	var bcode = HY.checkCntnoCode(HY.fv(frm, 'cntno'), function(isValid, num){
		if (isValid === true) {
	...
	HY.confirm ({
		callback: function() {
			var params = frm.getValues();
			Ext.apply(params, {"cntcode": Ext.getCmp('eld_cntcode').getRawValue(), "packing": Ext.getCmp('eld_packing').getRawValue()});
			HY.AjaxRequest({
				params: params,
				showWat: true,
				url: me.ctx + '/cntsave.do',
				callback: function(datobj) {
					HY.fv(frm, 'cntblid', datobj);
					me.doRefreshDb();
					me.dbStore.load();
				}
			});
...
blchkView.doModifyDb = function(){
	var me = this;
	var frm = me.dbEditWin.formPanel.getForm();
	var rec = me.mainWindow.gridPanel.getSelectionModel().getSelected();
	if( rec ) {
		var pkval = rec.get('cntblid');
		var lnkid = rec.get('cntsrcid');
		me.doLoadDetailDb(pkval, frm, lnkid);
		me.dbEditWin.show();
	}
};
blchkView.doDeleteDb = function(){
	var rec = this.mainWindow.gridPanel.getSelectionModel().getSelected();
	if( rec ) {
		var pkval = rec.get('cntblid');
		var fkval = rec.get('shpblid');
		if (!HY.isNumber(pkval) || pkval == '0') {
		...
		HY.confirm ({
			showWat: true,
			callback: function() {
				var url = blchkView.ctx + '/cntdel/' + pkval + '.do?shpblid=' + fkval;
				HY.AjaxRequest({url: url, showMsg: true, callback: function(datobj){
					blchkView.dbStore.remove(rec);
				}});
			}
		});
	}
};
blchkView.doLoadDetailDb = function(pkval, frm, lnkid) {
	if (!HY.isNumber(pkval)) {
		HY.alert("参数有误，请重试。");
		return;
	}
	var url = blchkView.ctx + '/cntget/' + pkval + '.do';
...
blchkView.doCloseDb = function() {
	var me = this;
	me.dbEditWin.hide();
};
blchkView.doNavDb = function(inc) {
	var me = this;
...
//另存为模板
blchkView.doSaveAsTmpl = function() {
	var me = this;
	var frm = me.mainWindow.formPanel.getForm();
	Ext.Msg.prompt("另存为模板", "请填写模板名称", function(btn, text) {
		if (btn == 'ok') {
			...
			var srcid = HY.fv(frm, 'shpblid');
			var params = {"tmplname": text.trim(), "srcid": srcid};
			HY.AjaxRequest({url: me.ctx + '/sastmpl.do', params: params, showMsg: true, callback: function(datobj){
...
//显示模板选择窗口
blchkView.doShowTmplList = function() {
	blchkView.popTemplates({callback: function(record){
		HY.confirm ({
			msg: "是否使用所选模板填充补料单？",
			showWat: true,
			callback: function() {
				if (record) {
					var excols = ',shpblid,jobid,shpid,clientid,';
					var frm = blchkView.mainWindow.formPanel.getForm();
...
//					HY.fv(frm ,'cnorname' ,record.data['cnorname']);
//					HY.fv(frm ,'cneename' ,record.data['cneename']);
//					HY.fv(frm ,'notifyparty' ,record.data['notifyparty']);
//					HY.fv(frm ,'notifyparty2' ,record.data['notifyparty2']);
				}
			}
		});
	}});
...
//提单确认
blchkView.doConfirm = function(){
	var me = this;
	var frm = me.mainWindow.formPanel.getForm();
	var pkval = HY.fv(frm, 'shpblid');
	if (!HY.isNumber(pkval) || pkval == "0") {
		HY.alert('请先保存数据！');
		return;
	}
...
//取消提单确认
blchkView.doUnconfirm = function(){
	var me = this;
	var frm = me.mainWindow.formPanel.getForm();
	var pkval = HY.fv(frm, 'shpblid');
	if (!HY.isNumber(pkval) || pkval == "0") {
...
blchkView.popTemplates = function(configs){
	var win_id = 'tmpl';
	HY.Cmp.createPopQuery({
		id: 'ppw_' + win_id
		,pageSize: 0
		,width: 618
		,dataUrl: blchkView.ctx + "/tmplst.do"
		,dataParams: {}
 ...
 * @param {String} type
 */
blchkView.clearPop = function(type) {
	var frm = blchkView.mainWindow.formPanel.getForm();
	var frmDb = blchkView.dbEditWin.formPanel.getForm();
	if (type == 'portrec') {
 ...
 * 收货港口-弹窗回调函数
 */
blchkView.setPortRec = function(record) {
	if (record) {
		var frm = blchkView.mainWindow.formPanel.getForm();
 ...
 * 装货港口-弹窗回调函数
 */
blchkView.setPortPol = function(record) {
	if (record) {
		var frm = blchkView.mainWindow.formPanel.getForm();
 ...
 * 中转港口-弹窗回调函数
 */
blchkView.setPortPot = function(record) {
	if (record) {
		var frm = blchkView.mainWindow.formPanel.getForm();
 ...
 * 卸货港口-弹窗回调函数
 */
blchkView.setPortPod = function(record) {
	if (record) {
		var frm = blchkView.mainWindow.formPanel.getForm();
 ...
 * 船公司-弹窗回调函数
 */
blchkView.setShipper = function(record) {
	if (record) {
		var frm = blchkView.mainWindow.formPanel.getForm();
 ...
 * 主单包装-弹窗回调函数
 */
blchkView.setPacking = function(record) {
	if (record) {
		var frm = blchkView.mainWindow.formPanel.getForm();
